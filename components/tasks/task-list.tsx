      ...
      <Card>
        <CardContent className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Tato zakázka zatím nemá žádné úkoly. Vytvořte první úkol pro začátek práce.
          </p>
          <Button>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Progress 
                value={task.progress} 
                className="h-2"
                // @ts-ignore - Progress component might not support color prop
                style={{ 
            ...
            <div className="text-xs text-gray-500 space-y-1">
              {task.startDate && (
                <div className="flex items-center">
                  ...
                  Začátek: {new Date(task.startDate).toLocaleDateString("cs-CZ")}
                </div>
              )}
              {task.endDate && (
                <div className="flex items-center">
                  <Calendar className="h-3 w-3 mr-1" />
                  Konec: {new Date(task.endDate).toLocaleDateString("cs-CZ")}
                </div>
              )}
            </div>
            ...
            <div className="pt-2 border-t">
              <div className="text-xs text-gray-500">
                Zakázka: <span className="font-medium">{task.contract.name}</span>
              </div>
            </div>
